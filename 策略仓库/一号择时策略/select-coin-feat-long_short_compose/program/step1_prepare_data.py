...
def prepare_data(conf):
    ...
    for file_path in spot_path.rglob('*-USDT.csv'):
        if is_trade_symbol(file_path.stem):
            spot_symbol_map[file_path.stem] = file_path
    print(f'📂 读取到的spot交易对数量：{len(spot_symbol_map)}')

    # 处理swap数据
    swap_symbol_map = {}
    for file_path in swap_path.rglob('*-USDT.csv'):
        if is_trade_symbol(file_path.stem):
            swap_symbol_map[file_path.stem] = file_path
    print(f'📂 读取到的swap交易对数量：{len(swap_symbol_map)}')
    ...
    if not {'spot', 'mix'}.isdisjoint(conf.select_scope_set):
        ...
        with ProcessPoolExecutor() as executor:
            futures = {executor.submit(preprocess_kline, file_path, True): symbol for symbol, file_path in
                       spot_symbol_map.items()}
            for future in tqdm(as_completed(futures), total=len(spot_symbol_map), desc='💼 处理spot数据'):
                try:
                    data = future.result()
    ...
    if not {'swap', 'mix'}.isdisjoint(conf.select_scope_set) or not {'swap'}.isdisjoint(conf.order_first_set):
        ...
        with ProcessPoolExecutor() as executor:
            futures = {executor.submit(preprocess_kline, file_path, False): symbol for symbol, file_path in
                       swap_symbol_map.items()}
            for future in tqdm(as_completed(futures), total=len(swap_symbol_map), desc='💼 处理swap数据'):
                try:
                    data = future.result()
                    symbol = futures[future]
    ...
    # ====================================================================================================
    print('ℹ️ 预处理行情数据...')
    if not spot_candle_data_dict and not swap_candle_data_dict:
        raise ValueError("❌ 未读取到任何交易数据（Spot 或 Swap 均为空）。请检查 config.py 中的数据路径配置，以及数据文件夹中是否包含符合要求的 CSV 文件。")
